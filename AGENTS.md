...
- После каждого изменения кода обязательно гоняем тесты (`uv run pytest -q`), хотя бы коротким прогоном.
- При исправлении багов и добавлении фич стремимся к test-first: сначала формулируем/пишем падающий тест, затем чиним/добавляем код, добиваемся зелёного.
